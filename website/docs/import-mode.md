...
curl -s -X PUT -H "$CT" -d '{"mode":"READWRITE"}' $SR/mode/my-subject
# {"mode":"READWRITE"}
```

`DELETE /mode/{subject}` also works here and is usually the better choice — see
[Leaving import mode](#leaving-import-mode) for how the two differ.
...
# 409 {"error_code":40901,"message":"Cannot import since found existing subjects"}
...
- Compatibility checks are skipped entirely. The subject's compatibility level is ignored for
  the duration of the import. This matters even though the target starts empty: a check
  compares against whatever was imported earlier in the same session, so the second version
  you import would otherwise be checked against the first. With `force=true` it matters more,
  since the subject can already hold live schemas.
...
curl -s -X POST -H "$CT" -d '{"schema":"{\"type\":\"int\"}","id":1001,"version":2}' \
  $SR/subjects/s/versions
# 409 {"error_code":40901,"message":"Overwrite new schema with id 1001 is not permitted."}
```
...
curl -s -X POST -H "$CT" -d '{"schema":"{\"type\":\"int\"}","id":200,"version":5}' \
  $SR/subjects/s/versions
# 409 {"error_code":40901,"message":"Subject 's' version 5 is already registered with schema
...
| ------------------------------------------------------ | ----- | ---------------------------------------------------------- |
| Subject has live schemas when entering import mode     | 40901 | Rejected unless `force=true`                               |
| `id` or `version` supplied while not in import mode    | 42205 | Rejected                                                   |
| `id` omitted while in import mode                      | 42205 | Rejected                                                   |
| `id` is already registered with different content      | 40901 | Rejected, IDs are immutable                                |
| `version` is already used by a different ID or content | 40901 | Rejected                                                   |
...
The first three digits of a code are the HTTP status that carries it. A `409` means the request
collides with something the registry already holds, so retrying it unchanged cannot help: pick a
free ID or version, or import the content you already bound to that ID. A `422` means the
request itself was not acceptable, usually because the mode and the body disagree.
...
- Importing the same ID from two source registries that had bound it to different schemas
  cannot succeed. The second import is rejected with `40901`.
...
If the duplicate IDs end up in the **same** subject, that subject's answer moves to the later
one. Both versions stay addressable and keep the ID they were registered with, but the
subject's content lookup is overwritten by the second import, so a later registration of that
content reports the later ID and creates no new version. Importing a given schema at most once
per subject avoids this entirely.
...
- **Schema metadata and rules.** A register request may carry `metadata` and `ruleSet` fields.
  Karapace accepts both and discards them without an error, so a schema that depends on
  metadata or rules attached alongside it arrives stripped. Check the source for these fields
  before you rely on the copy being faithful.
- **Configuration other than the compatibility level.** Import mode carries schemas, not
  configuration: replay `PUT /config` and `PUT /config/{subject}` yourself once the schemas are
  in. The compatibility level is the only setting Karapace stores, so anything else your source
  registry keeps per subject has no equivalent here.
- **Contexts and schema linking.** Karapace has no context or exporter endpoints. A subject
  name that carries a context prefix is an ordinary subject name with no special meaning, and
  there is no server-driven copy, so the migration has to be run by a client that reads from
  the source and registers here.
- **The `_schemas` topic itself.** Copy through the API rather than by replicating the topic.
  The reader understands the `CONFIG`, `SCHEMA`, `DELETE_SUBJECT`, `NOOP` and `MODE` record
  types; another registry's topic may hold types it does not, and with
  `kafka_schema_reader_strict_mode` enabled an unknown record type stops the reader.
...
- **Duplicate IDs.** One schema may be imported under more than one ID by default. Set
  ...
  with content.
- **References and deletions.** References are resolved in import mode as they are everywhere
  else, so import a referenced schema before the schema that points at it. A soft deleted
  version in the source is not reproduced by importing alone: import the version, then delete
  it.

...
// Modifications made by Aiven Ltd, 2024:
// - Use fully qualified name for field types
// - Always sort options by name
...
func Format(schema s.Schema) (s.Schema, error) {
	...
	walk.DescriptorProtos(res.FileDescriptorProto(), func(fn protoreflect.FullName, m proto.Message) error {
		astNode := res.Node(m)
		astNodeMapping[astNode] = fn
		return nil
	})

	fieldTypeMapping := map[protoreflect.FullName]string{}

	walk.Descriptors(res, func(d protoreflect.Descriptor) error {
		fd, ok := d.(protoreflect.FieldDescriptor)
		if ok {
			message := fd.Message()
			if message != nil {
				fieldTypeMapping[fd.FullName()] = string(message.FullName())
				return nil
			}
			enum := fd.Enum()
			if enum != nil {
				fieldTypeMapping[fd.FullName()] = string(enum.FullName())
				return nil
			}
		}
		return nil
	...
	fileNode := res.FileNode().(*ast.FileNode)
	f := newFormatter(&writer, fileNode, astNodeMapping, fieldTypeMapping)
	f.Run()
	newSchema := schema
	newSchema.Schema = writer.String()
...
func (f *formatter) writeFileHeader() {
	...
	sort.Sort(ByOptionName(optionNodes))
...
func (f *formatter) writeMessage(messageNode *ast.MessageNode) {
	...
	if len(messageNode.Decls) != 0 {
		elementWriterFunc = func() {
			writeNodes(messageNode.Decls, f)
		}
	}
...
func (f *formatter) writeField(fieldNode *ast.FieldNode) {
	...
	// to it.
	n := f.astNodeMapping[fieldNode]
	fullType := f.fieldTypeMapping[n]
	var t ast.IdentValueNode
	if fullType == "" {
		t = fieldNode.FldType
	} else {
		t = ast.NewIdentNode(fullType, fieldNode.FldType.Start())
	}
...
func (f *formatter) writeService(serviceNode *ast.ServiceNode) {
	...
	if len(serviceNode.Decls) > 0 {
		elementWriterFunc = func() {
			writeNodes(serviceNode.Decls, f)
		}
	}
...
func (f *formatter) writeCompactOptions(compactOptionsNode *ast.CompactOptionsNode) {
	...
	if len(compactOptionsNode.Options) > 0 {
		elementWriterFunc = func() {
			sort.Sort(ByOptionName(compactOptionsNode.Options))
...
func (f *formatter) writeStart(node ast.Node) {
	f.writeStartMaybeCompact(node, true)
}
...
type ByOptionName []*ast.OptionNode

func (a ByOptionName) Len() int      { return len(a) }
func (a ByOptionName) Swap(i, j int) { a[i], a[j] = a[j], a[i] }
func (a ByOptionName) Less(i, j int) bool {
	// The default options (e.g. cc_enable_arenas) should always
	// be sorted above custom options (which are identified by a
	// leading '(').
	left := stringForOptionName(a[i].Name)
	right := stringForOptionName(a[j].Name)
...
// writeNodes writes nodes with sorted options.
func writeNodes[T ast.Node](nodes []T, f *formatter) {
	optionNodes := []*ast.OptionNode{}
	for _, node := range nodes {
		if option, ok := ast.Node(node).(*ast.OptionNode); ok {
			optionNodes = append(optionNodes, option)
		}
	}
	...
	for _, node := range nodes {
		if _, ok := ast.Node(node).(*ast.OptionNode); !ok {
			f.writeNode(node)
		}
	}
}

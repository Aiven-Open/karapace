...
func (f *formatter) writeMessage(messageNode *ast.MessageNode) {
	...
	if len(messageNode.Decls) != 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(messageNode.Decls))
			for i, n := range messageNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeEnum(enumNode *ast.EnumNode) {
	...
	if len(enumNode.Decls) > 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(enumNode.Decls))
			for i, n := range enumNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeExtend(extendNode *ast.ExtendNode) {
	...
	if len(extendNode.Decls) > 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(extendNode.Decls))
			for i, n := range extendNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeService(serviceNode *ast.ServiceNode) {
	...
	if len(serviceNode.Decls) > 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(serviceNode.Decls))
			for i, n := range serviceNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeRPC(rpcNode *ast.RPCNode) {
	...
	if len(rpcNode.Decls) > 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(rpcNode.Decls))
			for i, n := range rpcNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeOneOf(oneOfNode *ast.OneofNode) {
	...
	if len(oneOfNode.Decls) > 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(oneOfNode.Decls))
			for i, n := range oneOfNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeGroup(groupNode *ast.GroupNode) {
	...
	if len(groupNode.Decls) > 0 {
		elementWriterFunc = func() {
			nodes := make([]ast.Node, len(groupNode.Decls))
			for i, n := range groupNode.Decls {
				nodes[i] = n
			}
			f.writeNodes(nodes)
		}
...
func (f *formatter) writeCompactOptions(compactOptionsNode *ast.CompactOptionsNode) {
	...
	if len(compactOptionsNode.Options) > 0 {
		elementWriterFunc = func() {
			sort.Slice(compactOptionsNode.Options, func(i, j int) bool {
				// The default options (e.g. cc_enable_arenas) should always
				// be sorted above custom options (which are identified by a
				// leading '(').
				left := stringForOptionName(compactOptionsNode.Options[i].Name)
				right := stringForOptionName(compactOptionsNode.Options[j].Name)
				if strings.HasPrefix(left, "(") && !strings.HasPrefix(right, "(") {
					// Prefer the default option on the right.
					return false
				}
...
func (f *formatter) writeNodes(nodes []ast.Node) {
	optionNodes := []*ast.OptionNode{}
	for _, node := range nodes {
		if option, ok := node.(*ast.OptionNode); ok {

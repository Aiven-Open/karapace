...
# Consumer setup - 100 JSON topics
NUM_TOPICS = 100
...
async def create_consumer_instance(base_url: str):
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        ...
        if r.status_code in (200, 409):
            print(f"✅ JSON consumer instance '{CONSUMER_INSTANCE}' ready in group '{CONSUMER_GROUP}'")
        else:
            raise RuntimeError(f"❌ Failed to create consumer: {r.status_code} - {r.text}")


async def subscribe_to_topics(base_url: str):
    """Subscribe the consumer instance to all topics."""
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        ...
        if r.status_code == 204:
            print(f"✅ Subscribed to {len(TOPICS)} JSON topics: {TOPIC_PREFIX}-0 to {TOPIC_PREFIX}-{len(TOPICS)-1}")
        else:
...
async def measure_consume_latency(base_url: str, total_messages: int):
    ...
    # Add timeout parameter to avoid waiting default 11 seconds
    # Use max_bytes to control batch size for better throughput
        ...
        while len(consumed_messages) < total_messages:
            ...
            if messages:
                ...
                if len(consumed_messages) % 5000 < len(messages):  # Print every ~5000 messages
                    print(f"Fetched {len(messages)} JSON messages, total {len(consumed_messages)}")
...
async def run_benchmark():
    results = []
    print(f"\n🔹 Benchmarking JSON consume from {VERSION} at {BASE_URL}{BASE_CONSUMER_URL}/records")
    latencies, responses = await measure_consume_latency(BASE_URL, total_messages=TOTAL_MESSAGES)
    avg = mean(latencies)
    p95 = sorted(latencies)[int(0.95 * len(latencies)) - 1]

    results.append((VERSION, f"{TOPIC_PREFIX} (100 topics)", BASE_CONSUMER_URL, avg, p95, sum(len(r) for r in responses)))
...
async def main():
    ...
    print(f"\n🔹 Setting up JSON consumer for {VERSION} at {BASE_URL}")

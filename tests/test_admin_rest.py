...
def test_admin_client(admin_client, producer):
    ...
    assert set(retrieved_names) == set(topic_names), "Returned value %r differs from written one %r" % \
                                                     (retrieved_names, topic_names)
    assert len(topic_info["brokers"]) == 1, "Only one broker during tests"
    for v in topic_info["topics"].values():
...
async def check_topics(c, topic_names):
    ...
    data = res.json()
    assert set(topic_names) == set(data), "Retrieved topic names do not match: %r" % data

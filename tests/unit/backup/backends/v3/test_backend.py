...
@pytest.mark.xfail(
    raises=IndexError,
    reason="Enum symbol defaults broken in upstream: https://issues.apache.org/jira/browse/AVRO-3760",
    strict=True,
)
...
# TODO: Remove this test and remove the xfail from
#  test_reader_raises_unknown_checksum_algorithm once fixed in upstream avro.
def test_reader_raises_index_error() -> None:
    topic_name = "a-topic"
    # This backup file was generated by temporarily adding an algorithm to the
    # ChecksumAlgorithm enum, and writing the backup using it.
    backup_path = Path(__file__).parent / "test_data" / "backup_v3_future_algorithm" / f"{topic_name}.metadata"

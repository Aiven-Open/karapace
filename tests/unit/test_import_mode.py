...
from karapace.core.errors import InvalidMode, OperationNotPermittedInMode, SchemaIdDoesNotMatch
...
class TestSetModeLocal:
    ...
    async def test_import_mode_with_force_preserves_existing_schemas(self) -> None:
        """force skips the emptiness check only; it must never delete schemas."""
        _, db, sent, set_mode_local = _make_registry_for_set_mode()
        db.insert_subject(subject=Subject("s"))
        db.insert_schema_version(
            subject=Subject("s"),
        ...
        assert len([m for m in sent if m[0] == "mode"]) == 1
        assert db.find_subject_schemas(subject=Subject("s"), include_deleted=False)
    ...
    async def test_subject_import_mode_allowed_for_unknown_subject(self) -> None:
        """Subject-level IMPORT is the entry point for a migration into a new subject."""
        _, _, sent, set_mode_local = _make_registry_for_set_mode()
        result = await set_mode_local(Mode.import_mode, subject=Subject("brand-new"))
        assert result == Mode.import_mode
        assert ("mode", Mode.import_mode, Subject("brand-new")) in sent

    async def test_global_import_mode_fails_if_any_subject_exists(self) -> None:
        _, db, _, set_mode_local = _make_registry_for_set_mode()
        db.insert_subject(subject=Subject("s"))
        db.insert_schema_version(
            subject=Subject("s"),
    ...
    async def test_global_import_mode_with_force_preserves_existing_schemas(self) -> None:
        _, db, sent, set_mode_local = _make_registry_for_set_mode()
        db.insert_subject(subject=Subject("s"))
        db.insert_schema_version(
            subject=Subject("s"),
        ...
        result = await set_mode_local(Mode.import_mode, force=True)
        assert result == Mode.import_mode
        assert [m for m in sent if m[0] == "schema"] == []
        assert db.find_subject_schemas(subject=Subject("s"), include_deleted=False)

    async def test_global_readwrite_mode_allowed_with_existing_subjects(self) -> None:
        _, db, _, set_mode_local = _make_registry_for_set_mode()
        db.insert_subject(subject=Subject("s"))
        db.insert_schema_version(
            subject=Subject("s"),
        ...
        assert await set_mode_local(Mode.readwrite) == Mode.readwrite
...
class TestWriteNewSchemaLocalImportMode:
    ...
    async def test_explicit_id_bound_to_other_content_is_rejected(self) -> None:
        """An id already mapped to different content cannot be rebound."""
    ...
    async def test_explicit_id_replay_of_same_content_is_allowed(self) -> None:
        """Re-importing identical content under the same id is idempotent, not a conflict."""
        _, db, sent, write_new = _make_registry_for_write()
        db.insert_subject(subject=Subject("s"))
        db.set_subject_mode(subject=Subject("s"), mode=Mode.import_mode)
        db.insert_schema_version(
        ...
        assert result == 7
        assert sent[-1]["schema_id"] == 7

    async def test_same_id_and_content_shared_across_subjects_is_allowed(self) -> None:
        """Several subjects may legitimately reference one schema id."""
        _, db, sent, write_new = _make_registry_for_write()
...
class TestControllerSetGlobalMode:
    ...
    async def test_force_flag_forwarded_to_registry(self) -> None:
        controller, registry, _ = _make_controller()
        registry.set_mode_local = AsyncMock(return_value=Mode.import_mode)
        await controller.set_global_mode(mode_request=ModeUpdateRequest(mode="IMPORT"), force=True)
        registry.set_mode_local.assert_awaited_once_with(mode="IMPORT", force=True)
...
class TestControllerSetSubjectMode:
    ...
    async def test_unknown_subject_is_allowed(self) -> None:
        """Setting a subject mode must not require the subject to exist yet."""
        controller, registry, _ = _make_controller()
        registry.set_mode_local = AsyncMock(return_value=Mode.import_mode)
        resp = await controller.set_subject_mode(subject="ghost", mode_request=ModeUpdateRequest(mode="IMPORT"))
        assert resp.mode == "IMPORT"

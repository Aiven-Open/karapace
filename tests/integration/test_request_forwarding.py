...
@pytest.fixture(scope="function", name="request_forwarding_retry_client_tls")
async def fixture_registry_async_client_tls(
    server_ca: str,
    registry_async_pair_tls: list[str],
) -> AsyncGenerator[RetryRestClient, None]:
    client = Client(server_uri=registry_async_pair_tls[0], server_ca=server_ca)
...
@pytest.mark.parametrize("subject", ["test_forwarding", "test_forw/arding"])
async def test_schema_request_forwarding_tls(
    registry_async_pair_tls: list[str],
    ...
    for subj_path in [None, subject]:
        ...
        for compat in ["FULL", "BACKWARD", "FORWARD", "NONE"]:
            ...
            while True:
                assert counter < max_tries, "Compat update not propagated"
                resp = await request_forwarding_retry_client_tls.get(f"{master_url}/{path}")
                if not resp.ok:
                    print(f"Invalid http status code: {resp.status_code}")
                    continue
                data = resp.json()
    ...
    while True:
        assert counter < max_tries, "Subject schema data not propagated yet"
        resp = await request_forwarding_retry_client_tls.get(f"{master_url}/subjects/{quote_plus(subject)}/versions")
        if not resp.ok:
            print(f"Invalid http status code: {resp.status_code}")
            counter += 1
            continue
    ...
    while True:
        assert counter < max_tries, "Subject version deletion not propagated yet"
        resp = await request_forwarding_retry_client_tls.get(
            f"{master_url}/subjects/{quote_plus(subject)}/versions/1", expected_response_code=404
        )
        if resp.ok:
            print(f"Subject {subject} still has version 1 on master")
    ...
    # Subject deletion
    resp = await request_forwarding_retry_client_tls.get(f"{master_url}/subjects/")
    assert resp.ok
    data = resp.json()
    assert subject in data

"""Copyright (c) 2023 Aiven Ltd See LICENSE for details This version of the tests applies the same schemas as in
test_jsonschema.py, but these schemas are embedded within an additional helper schema, which is then referenced by
another schema. This setup allows us to test the behavior of JSON schema references"""
...
async def sainty_check(client: Client, subject: str, compatibility_mode: CompatibilityModes) -> None:
    # Sanity check. The compatibility must be explicitly set because any
    # difference can result in unexpected errors.
    subject_config_res = await client.get(f"config/{subject}_dependency?defaultToGlobal=true")
...
async def not_schemas_are_compatible(
    ...
    subject_res = await client.get(f"subjects/{subject}_dependency/versions")
    assert subject_res.status_code == 404, "random subject should no exist {subject}"

    template_schema = {
        "$id": "https://example.com/dependency.schema.json",
        "title": "Dependency",
    }
    older_schema = dict(template_schema)
    older_schema.update(older.schema)
    older_dependency_res = await client.post(
        f"subjects/{subject}_dependency/versions",
        json={
            "schema": json.dumps(older_schema),
    ...
    main_schema = {
        ...
        "properties": {
            "x1": {"type": "string"},
            "x2": {"type": "string"},
            "x3": {"$ref": "https://example.com/dependency.schema.json"},
        },
        "required": ["x1", "x2", "x3"],
    }

    main_res = await client.post(
    ...
    newer_schema.update(newer.schema)
    newer_res = await client.post(
        f"subjects/{subject}_dependency/versions",
        json={
            "schema": json.dumps(newer_schema),
            "schemaType": SchemaType.JSONSCHEMA.value,
        },
    )

...
def setup_middlewares(app: FastAPI, config: Config) -> None:
    oidc_validator = OIDCValidator(config)
    ...
    async def set_content_types(request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        if auth_header and auth_header.startswith("Bearer "):
            try:
                payload = oidc_validator.validate_jwt(auth_header.split(" ", 1)[1])
                request.state.user = payload  # You can store this payload for use in route handlers

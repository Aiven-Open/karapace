...
https://github.com/advboxes/perceptron-benchmark/blob/master/perceptron/utils/protobuf/protobuf_to_dict.py
License: (Apache 2.0) https://github.com/advboxes/perceptron-benchmark/blob/master/LICENSE
"""
...
def protobuf_to_dict(pb, use_enum_labels=False):
    """ protobuf to dict
...
def _get_field_value_adaptor(pb, field, type_callable_map, use_enum_labels=False):

    if field.type == FieldDescriptor.TYPE_MESSAGE:
        # recursively encode protobuf sub-message
        return lambda pb: protobuf_to_dict(pb, use_enum_labels)
...
def dict_to_protobuf(pb_klass_or_instance, values, strict=True):
    ...
    # dict type_callable_map: a mapping of protobuf types to callables for setting
...
def _get_field_mapping(pb, dict_value, strict):
    ...
    for ext_num, ext_val in dict_value.get(EXTENSION_CONTAINER, {}).items():
        ...
        if ext_num not in pb._extensions_by_number:
            if strict:
                raise KeyError(
                    "%s does not have a extension with number %s. Perhaps you forgot \
                    to import it?" % (pb, key)
                )
            continue
        # pylint: disable=protected-access
    ...
    for field, input_value, pb_value in fields:
        if field.label == FieldDescriptor.LABEL_REPEATED:
            for item in input_value:
                ...
                elif field.type == FieldDescriptor.TYPE_ENUM and isinstance(item, (str, bytes)):
        ...
        if field.type == FieldDescriptor.TYPE_ENUM and isinstance(input_value, (str, bytes)):

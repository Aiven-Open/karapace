...
from karapace.typing import SchemaId, SubjectData
...
class KafkaSchemaReader(Thread):
    def __init__(
        ...
        *,
        ...
        self.schema_record_producer = schema_record_producer
        self.timeout_ms = 200
        self.config = config
    ...
    def get_schema_id_if_exists(self, *, subject: Subject, schema: ValidatedTypedSchema) -> Optional[SchemaId]:
        with self.id_lock:
            sid = self._hash_to_schema_id_on_subject.get(hash((subject, schema.__str__())), None)
            if sid is not None:
                return sid
            # In case if the hashing does not return id check with parsed schema if
            # equivalent schema can be found. In Avro e.g. "string" == {"type":"string"}.
            # If found set the equivalent schema to cache.
            maybe_id = None
            for existing_id, existing_schema in self.schemas.items():
                validated = ValidatedTypedSchema.of(existing_schema)
                if existing_schema.schema_type is SchemaType.JSONSCHEMA:
                    if validated.to_dict() == schema.to_dict():
                        maybe_id = existing_id
                elif validated.schema == schema.schema:
                    maybe_id = existing_id

            if maybe_id is not None:
                schemas = self.subjects.get(subject, {}).get("schemas")
                if schemas:
                    maybe_schema = next(
                        (
                            existing_schema
                            for existing_schema in schemas.values()
                            if ValidatedTypedSchema.of(existing_schema.get("schema")).schema == schema.schema
                        ),
                        None,
                    )
                    if maybe_schema is not None:
                        self._set_schema_id_on_subject(subject=subject, schema=schema, schema_id=maybe_id)
                        return maybe_id
            return None
    ...
    def handle_messages(self) -> None:
        ...
        for _, msgs in raw_msgs.items():
            ...
            for msg in msgs:
                ...
                self.handle_msg(key, msg.key, value)
                ...
                if self.ready and watch_offsets:
                    self.offsets_watcher.offset_seen(self.offset)
    ...
    def _handle_msg_schema(self, key: dict, key_bytes: bytes, value: Optional[dict]) -> None:
        """Handle schema message

        :param key: The key dictionary
        :param key_bytes: The original byte representation of key.
                         Used when rollback record is sent in case of duplicate id.
        :param value: The schema record value
        """
        if not value:
        ...
        if not self.ready and self.schemas.get(schema_id):
            existing_schema = self.schemas.get(schema_id)
            if existing_schema != typed_schema:
                LOG.warning(
                    (
                        "Schema found with id '%s', skipping and not registering this schema. "
                        "Sending rollback record for duplicate record with key '%s'."
                    ),
                    schema_id,
                    key_bytes,
                )
                self.schema_record_producer.send_kafka_message(
                    key=key_bytes,
                    value=None,
                    wait_for_offset=False,
                ).get()
                return
        ...
        try:
            schema_str = typed_schema.normalize_schema_str()
        except (json.JSONDecodeError, InvalidSchema):
            return
    ...
    def handle_msg(self, key: dict, key_bytes: bytes, value: Optional[dict]) -> None:
        if key["keytype"] == "CONFIG":
            self._handle_msg_config(key, value)
        elif key["keytype"] == "SCHEMA":
            self._handle_msg_schema(key, key_bytes, value)
